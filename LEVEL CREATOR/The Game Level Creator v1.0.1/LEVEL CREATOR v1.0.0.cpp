...
void SearchAndDestroyTP(Vector2 *pos,Block _block[][302],TeleportHash _tph[],int &_tpCount)
{
...
void DestroyActivationPoint(Block _block[][302],Vector2 *pos,Vector2 _atvPos[],int &_atvCount)
{
...
//Shape creator
void DrawLine(char _bIcon,Vector2 *Begin,Vector2 *End,Block _block[][302])
{
	int b_x = Begin->_x;
	int b_y = Begin->_y;
	int e_x = End->_x;
...
void DrawSquare(char _bIcon, int _length,Vector2 *Center,Block _block[][302])
{
...
void InitializeGame(Block _block[][302],Player *&_player)
{
	...
	for(int i=0;i<=300;i++)
		for(int j=0;j<=300;j++)
		{
			...
			_block[i][j]._blockType = Block_Ground;
			Vector2 *p = &_block[i][j]._position;
			p = new Vector2(i,j);
		}
	//Set icon for Blocks
	for(int i=0;i<=300;i++)
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
//Input Handler
void Input(bool &A_Quit,GameState &_gameState,Player *&_player,Block _block[][302],int currentLevel,char _fileName[],char &_sIcon,int &_selectNum,Player *&_p,TeleportHash &_editTP,TeleportHash _tph[],int &_tpCount,Vector2 _atvPos[],int &_atvCount)
{
	if(kbhit())
	{
		switch(getch())
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
//Data Handling
void LoadGame(char _fileName[50],FILE *_fptr,Player *&_player,Block _blockArr[][302],TeleportHash _tph[],int &_tpCount ,int &currentLevel,bool &_gameSaved,char _saveName[],int &_atvCount, Vector2 _atvPos[],Player *&_p)
{
...
void SaveGame(Block _blockArr[][302],FILE *_fptr,char _fileName[50],int _currentLevel,int _tpCount, TeleportHash _tph[],Player *_player,bool &_gameSaved,char _saveName[],int _atvCount,Vector2 _atvPos[],bool _createMap,Player *_p,TeleportHash &_editTP)
{
	//Editor clearing
	if((_editTP._pos1)->_x != DEFAULT_TP_RESET && (_editTP._pos1)->_y != DEFAULT_TP_RESET)
	{
...
void SaveProgress(int _index,Block _blockArr[][302],FILE *_fptr,char _fileName[50],int _currentLevel,int _tpCount, TeleportHash _tph[],Player *_player,bool &_gameSaved,char _saveName[],SavedFile _fileProperties[],int currentLevel,char _strArr [][50],int _atvCount,Vector2 _atvPos[],Player *_p,TeleportHash &_editTP)
{
...
void MainMenuUI(GameState &_gameState,bool &A_quit,char _fileName[50],SavedFile _fileProperties[],char _saveName[],Block _block[][302],Player *&_player,int &_currentLevel,int &_tpCount,int &_atvCount)
{
	int maxNum = 2;
	int currentNum = 0;
...
void PauseMenuUI(GameState &_gameState,bool &A_Quit,Block _blockArr[][302],FILE *_fptr,char _fileName[], int &currentLevel,int &_tpCount,TeleportHash _tph[],Player *_player,bool _gameSaved,SavedFile _fileProperties[],char _saveName[],int _atvCount,Vector2 _atvPos[],Player *_p,TeleportHash &_editTP)
{
	int maxNum = 4;
	int currentNum = 0;
	char _nameArr[maxNum+2][50];
						...
						case 1:
							{
								...
								{
									system("cls");
									printf("Enter Map Level (x>1,""-1"" to exit): ");
									scanf("%d",&_temp);
									if(_temp==-1)
										break;
									if(_temp<1)
...
void DrawGame(Window *_window,Block _block[][302],Player *_player,int currentLevel,/*Debug*/TeleportHash _tph[],int _tpCount,Vector2 _atvPos[],int _atvCount,int _selectNum,Player *_p)
{
	system("cls");
	//Game Window
	...
	if(_p != NULL)
		printf("\n %d %d", (_p->_position)._x,(_p->_position)._y);
